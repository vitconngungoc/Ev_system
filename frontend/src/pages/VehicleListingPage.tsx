...
interface VehicleListingPageProps {
  ...
  onBack: () => void;
  onSetFilterTimes?: (startDate: string, startTime: string, endDate: string, endTime: string) => void;
}
...
export function VehicleListingPage({ stationId, onNavigate, onBack, onSetFilterTimes }: VehicleListingPageProps) {
  ...
  const handleSearchAvailability = async () => {
    ...
    try {
      ...
      // Lưu thời gian đã lọc để truyền sang VehicleDetailPage
      if (onSetFilterTimes) {
        onSetFilterTimes(startDate, startTime, endDate, endTime);
        console.log('💾 Saved filter times:', { startDate, startTime, endDate, endTime });
      }
      
      if (availableModels.length === 0) {
        toast.success('Không tìm thấy xe khả dụng trong khung giờ này');
      } else {
