...
export function VehicleDetailPage({ 
  ...
  const initializeDates = () => {
    // Nếu có thời gian từ bộ lọc, sử dụng chúng
    if (filterStartDate && filterStartTime && filterEndDate && filterEndTime) {
      ...
      console.log('📅 Using filter times:', { filterStartDate, filterStartTime, filterEndDate, filterEndTime });
    } else {
  ...
  const handleCheckAvailability = async () => {
    ...
    try {
      ...
      if (!availabilityData.available) {
        // Nếu message từ backend là "Xe khả dụng trong khung giờ này." thì đổi thành success (xanh)
        const backendMessage = availabilityData.message || 'Xe đã có người đặt trong khung giờ này';
        if (backendMessage.includes('Xe khả dụng trong khung giờ này')) {
      ...
      toast.success('Xe khả dụng trong khung giờ này.');
    } catch (error) {
  ...
  const handleBooking = async () => {
    ...
    if (user.status === 'INACTIVE') {
      toast.error('Tài khoản của bạn đã bị khóa do hủy đơn quá nhiều lần. Vui lòng liên hệ quản trị viên để được hỗ trợ.', {
        duration: 6000,
      });
      return;
    }
