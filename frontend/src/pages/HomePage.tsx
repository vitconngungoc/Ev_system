...
import { authenticatedApiCall, API_ENDPOINTS, API_BASE_URL } from '../lib/api';
...
export function HomePage({ user, onNavigate, onLogout }: HomePageProps) {
  ...
  useEffect(() => {
    const fetchGlobalRating = async () => {
      try {
        // Lấy danh sách tất cả các trạm (public endpoint - không cần token)
        ...
        // Lấy rating của từng trạm (public endpoint)
        for (const station of stations) {
          try {
            const ratingsResponse = await fetch(
              `${API_BASE_URL}/stations/rating/${station.stationId}`,
              {
                method: 'GET',
                headers: {
                  'Content-Type': 'application/json',
  ...
  const handleShowAllRatings = async () => {
      ...
      for (const station of stations) {
        try {
          ...
          if (ratingsResponse.ok) {
            ...
            if (Array.isArray(ratings) && ratings.length > 0) {
              // Thêm thông tin trạm vào mỗi rating
              const ratingsWithStation = ratings.map((r: any) => ({
                ...r,
                stationName: station.name,
                stationAddress: station.address,
              }));
              allRatingsData.push(...ratingsWithStation);
            }
          }
        } catch (err) {
          console.error(`Failed to fetch ratings for station ${station.stationId}:`, err);
        }
      }
      
      // Sắp xếp theo thời gian mới nhất
      allRatingsData.sort((a, b) => 
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );
      
      setAllRatings(allRatingsData);
      console.log('📊 All ratings loaded:', allRatingsData.length);
    } catch (error) {
  ...
  const getVerificationBadge = () => {
    ...
    // Staff không hiển thị verification badge, chỉ hiển thị vai trò
    const isStaff = user.role?.roleId === 2 || user.role?.roleName === 'STATION_STAFF';
    const isAdmin = user.role?.roleId === 3 || user.role?.roleName === 'ADMIN' || user.role?.roleName === 'EV_ADMIN';
                  ...
                  <div className="relative">
                    ...
                    {showUserMenu && (
                      <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                        ...
                        <div className="py-2">
                          {/* Staff có nút quay lại Staff Dashboard */}
                          {(user.role?.roleId === 2 || user.role?.roleName === 'STATION_STAFF') ? (
                            <button
                              onClick={() => {
                            ...
                            >
                              <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center group-hover:bg-blue-600 transition-colors">
                                <UserIcon className="w-4 h-4 text-blue-600 group-hover:text-white transition-colors" />
                              </div>
                              <span className="text-sm text-gray-900 font-medium">Quay lại Dashboard</span>
                            </button>
                          ) : (user.role?.roleId === 3 || user.role?.roleName === 'ADMIN' || user.role?.roleName === 'EV_ADMIN') ? (
                            /* Admin có nút quay lại Admin Dashboard */
                            <button
                              onClick={() => {
                                setShowUserMenu(false);
                            ...
                            >
                              <div className="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center group-hover:bg-purple-600 transition-colors">
                                <UserIcon className="w-4 h-4 text-purple-600 group-hover:text-white transition-colors" />
                              </div>
                              <span className="text-sm text-gray-900 font-medium">Quay lại Dashboard</span>
                            </button>
                          ) : (
        ...
        <div className="relative max-w-7xl mx-auto px-8 text-center">
          <div className="max-w-3xl mx-auto">
            ...
            <div className="flex items-center justify-center gap-8 mt-16 text-white/60">
              <div 
                className="flex items-center gap-2 cursor-pointer hover:text-white/80 transition-colors"
