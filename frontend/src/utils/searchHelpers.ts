...
export const filterVehicles = <T extends Record<string, any>>(
  ...
  return vehicles.filter((vehicle) => {
    ...
    if (filters.seatingCapacity && vehicle.seatingCapacity < filters.seatingCapacity) return false;
    if (filters.location && vehicle.location !== filters.location) return false;
    if (filters.availability !== undefined && vehicle.available !== filters.availability) return false;
    return true;
  });
};

export const searchVehicles = <T extends Record<string, any>>(vehicles: T[], searchTerm: string): T[] => {
...
export const sortVehicles = <T extends Record<string, any>>(vehicles: T[], sortBy: SortOption): T[] => {
